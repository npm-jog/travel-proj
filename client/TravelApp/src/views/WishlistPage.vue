  ...
  <ion-page>
    <ion-content :fullscreen="true" id="main-content" ion-padding>
      <div class="wishlist-section">
        <h1 v-if="userInfo">{{ userInfo.username }}'s Wishlist</h1>
      </div>

      <ion-card v-for="(country, index) in userInfo.wishlist" :key="index" class="card">
        <ion-card-title class="title">{{ country }}</ion-card-title>
        <img :src="`${imagesCountry[index]}`" class="image-country" />
        <div class="button-container">
          <ion-button @click="navigateTo(`/country/${country}`)" class="button">View more</ion-button>

          <ion-button class="button" color="danger" @click="patchUser(`${country}`)">Remove</ion-button>
        </div>
      </ion-card>
    </ion-content>
  </ion-page>
  ...
  methods: {
    ...
    async loadImages() {
      const imagePromises = this.userInfo.wishlist.map((wish: any) => {
        return axios.get(`https://travel-app-api-8nj9.onrender.com/api/country_data/images/${wish}`);
      });
    ...
    async patchUser(country: string) {
      ...
      try {
        await axios.patch(`https://travel-app-api-8nj9.onrender.com/api/users/${this.userInfo._id}`, newUserInfo);
      } catch (err) {
        console.log(err);
      }
    },
